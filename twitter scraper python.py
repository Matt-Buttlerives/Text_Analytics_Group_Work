...
# Create a dataframe from the tweets list above 
tweets_df_keyword = pd.DataFrame(tweets_list_keyword, columns=['Datetime','Tweet Id', 'Text', 'Username', 'url'])
tweets_df_keyword.to_csv("tweets_keyword_bitcoin_surge.csv",index=False) # Export to a csv file
print("Scraped data have been exported to the csv file")
...
# Create a dataframe from the tweets list above 
tweets_df_keyword = pd.DataFrame(tweets_list_keyword, columns=['Datetime','Tweet Id', 'Text', 'Username', 'url'])
tweets_df_keyword.to_csv("tweets_keyword_bitcoin_crash.csv",index=False) # Export to a csv file
print("Scraped data have been exported to the csv file")
...
all_pairs = list(itertools.combinations(top_5_coins, 2))
...
top_5_coins = pd.Series(top_5_coins, name = 'Token')

coin_association=coin_association.merge(top_5_coins,left_on=['Coin1'],right_on=['Token'],how='left').drop(['Token'],axis=1)

coin_association=coin_association.merge(top_5_coins,left_on=['Coin2'],right_on=['Token'],how='left').drop(['Token'],axis=1)
